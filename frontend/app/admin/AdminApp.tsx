...
import type { AdminNavKey } from "./navConfig";
import { NAV_SECTIONS, getAdminSectionRoute } from "./navConfig";

const CUSTOMER_SEGMENTS = [
  { id: "all", label: "Semua pelanggan", description: "Seluruh database pelanggan aktif" },
  { id: "loyal", label: "Member Loyal", description: "Skor loyalitas tinggi & sering repeat order" },
  { id: "new", label: "Pelanggan Baru", description: "Mendaftar < 30 hari terakhir" },
  { id: "inactive", label: "Perlu Disapa", description: "Tidak transaksi > 60 hari" },
];

const DEFAULT_VOUCHERS = [
  { code: "AIVRA15", label: "Diskon 15% Signature Series", quota: 120, status: "Aktif" },
  { code: "AIVRAWEEKEND", label: "Bundling Weekend", quota: 60, status: "Terjadwal" },
  { code: "MEJA10", label: "Diskon Reservasi Meja", quota: 40, status: "Nonaktif" },
];

const TABLE_SERVICE_TASKS = [
  { title: "Sanitasi tiap 2 jam", owner: "Tim Housekeeping", status: "On Track" },
  { title: "Kalibrasi QR & NFC", owner: "Tim IT Store", status: "Due Today" },
  { title: "Audit perangkat kasir", owner: "Supervisor Shift", status: "Scheduled" },
];

const TABLE_RESERVATIONS = [
  { guest: "Rangga & Team", pax: 6, slot: "Hari ini, 19.00", table: "Area Window", note: "Meeting internal" },
  { guest: "Alya", pax: 2, slot: "Hari ini, 20.30", table: "Meja Bar", note: "Anniversary" },
  { guest: "Pak Budi", pax: 4, slot: "Besok, 10.00", table: "VIP Room", note: "Tap-in quick meeting" },
];

const BULK_CHANNELS: Array<{ key: BulkChannelKey; label: string; icon: string }> = [
...
const CUSTOMER_PROFILES = [
  { name: "Alya Rahmania", tier: "Gold", transactions: 42, favorite: "Pistachio Latte", lastVisit: "Kemarin • 20.15", spend: "Rp 1,2 jt" },
  { name: "Dimas Setiawan", tier: "Silver", transactions: 18, favorite: "Matcha Frappe", lastVisit: "2 hari lalu • 09.30", spend: "Rp 640 rb" },
  { name: "Nadia Putri", tier: "Member Baru", transactions: 5, favorite: "Es Kopi Gula Aren", lastVisit: "Hari ini • 08.10", spend: "Rp 210 rb" },
  { name: "Rizky Mahendra", tier: "Platinum", transactions: 73, favorite: "Single Origin Yirgacheffe", lastVisit: "3 hari lalu • 19.45", spend: "Rp 2,8 jt" },
];

const DEFAULT_TABLE_LAYOUT = [
  { name: "Meja 1", seats: 2, status: "available" },
  { name: "Meja 2", seats: 4, status: "reserved" },
  { name: "Meja 3", seats: 2, status: "cleaning" },
  { name: "Meja 4", seats: 4, status: "available" },
  { name: "Meja 5", seats: 6, status: "occupied" },
  { name: "Meja 6", seats: 2, status: "available" },
  { name: "Meja 7", seats: 4, status: "occupied" },
  { name: "Meja 8", seats: 2, status: "reserved" },
];
...
type BulkChannelKey = "whatsapp" | "email" | "sms";
...
type ProductCategory = CategorySlug;

type ProductCustomizationGroup = CustomizationGroup & {
...
const VARIANT_STORAGE_KEY = "aivra-admin-variants";

const VARIANT_TEMPLATE_SEEDS: Array<{
  key: string;
  label: string;
  description: string;
...
function createVariantFromSeed(seed: (typeof VARIANT_TEMPLATE_SEEDS)[number]): CustomizationGroup {
  return {
    id: createId(`variant-${seed.key}`),
    ...seed.group,
    options: seed.group.options.map((option, index) => ({
      id: option.id && option.id.trim() ? option.id : createId(`variant-opt-${seed.key}-${index}`),
      label: option.label,
...
function normalizeVariantTemplate(raw: CustomizationGroup, index = 0): CustomizationGroup {
  return {
    id: raw.id && raw.id.trim() ? raw.id : createId(`variant-${index}`),
    name: raw.name,
    ...
    options: Array.isArray(raw.options)
      ? raw.options.map((option, optionIndex) => ({
          id: option.id && option.id.trim() ? option.id : createId(`variant-opt-${index}-${optionIndex}`),
          label: option.label ?? `Pilihan ${optionIndex + 1}`,
...
function createDefaultVariantTemplates(): CustomizationGroup[] {
  return VARIANT_TEMPLATE_SEEDS.map((seed) => createVariantFromSeed(seed));
}

function loadVariantTemplatesFromStorage(): CustomizationGroup[] {
  if (typeof window === "undefined") {
    return createDefaultVariantTemplates();
  }
  try {
    ...
    return parsed.map((variant, index) => normalizeVariantTemplate(variant, index));
  } catch (error) {
    console.error("Gagal memuat varian dari storage", error);
    return createDefaultVariantTemplates();
  }
}

function createEmptyVariantForm(): CustomizationGroup {
  return {
    id: createId("variant"),
    name: "",
    type: "single",
    ...
    helperText: "",
    options: [
      { id: createId("variant-opt"), label: "Pilihan 1", priceAdjustment: 0 },
      { id: createId("variant-opt"), label: "Pilihan 2", priceAdjustment: 0 },
    ],
  };
}
...
const DEFAULT_ADMIN_SETTINGS: AdminSettings = {
  ...
  payment: {
    qrisMerchantName: "AIVRA",
    qrisId: "00020101021234567890",
    bankName: "BCA",
    bankAccountName: "AIVRA",
...
export type AdminAppProps = {
  initialSection?: AdminNavKey;
};

export function AdminApp({ initialSection = "overview" }: AdminAppProps) {
  ...
  const activeKey: AdminNavKey = initialSection ?? "overview";
  ...
  const [variantTemplates, setVariantTemplates] = useState<CustomizationGroup[]>(() =>
    loadVariantTemplatesFromStorage()
  );
  const [variantForm, setVariantForm] = useState<CustomizationGroup>(() => createEmptyVariantForm());
  ...
  const [voucherForm, setVoucherForm] = useState<VoucherFormState>({
    ...
    minSpend: "100000",
    expiry: "",
    description: "Diskon khusus pelanggan loyal",
  });
  ...
  const showVariantNotification = (message: string, duration = 2200) => {
    setVariantMessage(message);
    if (typeof window !== "undefined") {
      window.setTimeout(() => setVariantMessage(null), duration);
    }
  };
  const [dailyOrderTarget, setDailyOrderTarget] = useState(() => {
    ...
    const stored = window.localStorage.getItem("aivra-order-target");
  ...
  const handleVariantTextFieldChange = (field: "name" | "helperText", value: string) => {
    setVariantForm((prev) => ({
      ...prev,
      [field]: value,
    }));
  ...
  const handleVariantTypeChange = (value: CustomizationGroup["type"]) => {
    setVariantForm((prev) => ({
      ...prev,
      type: value,
    }));
  ...
  const handleVariantRequiredChange = (value: boolean) => {
    setVariantForm((prev) => ({
      ...prev,
      required: value,
    }));
  ...
  const handleVariantOptionFieldChange = (
    ...
    setVariantForm((prev) => ({
      ...
      options: prev.options.map((option) => {
        ...
        return { ...option, priceAdjustment: numeric };
      }),
    }));
  };
  ...
  const addVariantOptionField = () => {
    setVariantForm((prev) => ({
      ...
      options: [
        ...prev.options,
        {
          id: createId("variant-opt"),
          label: `Pilihan ${prev.options.length + 1}`,
          priceAdjustment: 0,
        },
      ],
    }));
  ...
  const removeVariantOptionField = (optionId: string) => {
    setVariantForm((prev) => {
      const nextOptions = prev.options.filter((option) => option.id !== optionId);
      return {
        ...prev,
        options: nextOptions.length > 0 ? nextOptions : prev.options,
      };
    });
  };

  const handleEditVariant = (variant: CustomizationGroup) => {
    setVariantForm({
      ...variant,
  ...
  const handleSaveVariant = () => {
    if (!variantForm.name.trim()) {
      showVariantNotification("Nama varian wajib diisi.");
      return;
    }
    const sanitizedOptions = variantForm.options
      .map((option, index) => ({
        id: option.id && option.id.trim() ? option.id : createId(`variant-opt-${index}`),
        label: option.label.trim(),
        priceAdjustment: Number(option.priceAdjustment) || 0,
      }))
      .filter((option) => option.label.length > 0);
    if (sanitizedOptions.length === 0) {
      showVariantNotification("Minimal harus ada satu pilihan pada varian.");
      return;
    }
    const normalized: CustomizationGroup = {
      id: editingVariantId ?? variantForm.id ?? createId("variant"),
      name: variantForm.name.trim(),
      type: variantForm.type,
      required: variantForm.required,
      helperText: variantForm.helperText.trim(),
      options: sanitizedOptions,
    };
    setVariantTemplates((prev) => {
      if (editingVariantId) {
        return prev.map((variant) => (variant.id === editingVariantId ? normalized : variant));
      }
      return [...prev, normalized];
    });
    resetVariantForm();
    showVariantNotification(editingVariantId ? "Varian diperbarui." : "Varian ditambahkan.");
  };

  const handleDeleteVariant = (variantId: string) => {
    setVariantTemplates((prev) => prev.filter((variant) => variant.id !== variantId));
    setProductForm((prev) => ({
      ...prev,
      customizations: prev.customizations.filter((group) => group.sourceVariantId !== variantId),
    }));
    if (editingVariantId === variantId) {
      resetVariantForm();
    }
    showVariantNotification("Varian dihapus.");
  };

  const applyVariantToProduct = (variantId: string) => {
    const template = variantTemplates.find((variant) => variant.id === variantId);
    if (!template) {
      return;
    }
    setProductForm((prev) => {
      if (prev.customizations.some((group) => group.sourceVariantId === variantId)) {
        return prev;
      }
      const clonedGroup: ProductCustomizationGroup = {
        id: createId("cst"),
        name: template.name,
        type: template.type,
      ...
      return {
        ...prev,
        customizations: [...prev.customizations, clonedGroup],
      };
    });
  };
  useEffect(() => {
    ...
    try {
      const stored = loadVariantTemplatesFromStorage();
      setVariantTemplates(stored);
    } catch (error) {
      console.error("Gagal memuat varian saat inisialisasi", error);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem(VARIANT_STORAGE_KEY, JSON.stringify(variantTemplates));
    } catch (error) {
      console.error("Gagal menyimpan varian ke storage", error);
    }
  }, [variantTemplates]);
  useEffect(() => {
    if (typeof window === "undefined") {
    ...
    try {
      window.localStorage.setItem("aivra-order-target", String(dailyOrderTarget));
    } catch (error) {
      console.error("Gagal menyimpan target pesanan", error);
    }
  }, [dailyOrderTarget]);
  ...
  const fulfillmentTarget = Math.max(dailyOrderTarget, 1);
  const fulfillmentRate = Math.min(
    100,
    Math.round(((readyOrders + servedOrders) / Math.max(fulfillmentTarget, 1)) * 100)
  );
  const collaborationMembers = (adminAccounts.length ? adminAccounts : getAdminAccountsForSettings()).slice(0, 4);
  const summaryCards = [
    {
      label: "Pesanan Aktif",
      value: String(pendingOrders + preparingOrders),
      icon: "receipt_long",
      detail: `${pendingOrders} menunggu • ${preparingOrders} diproses`,
    },
    {
      label: "Estimasi Pendapatan",
      value: "Rp 3.450.000",
      icon: "payments",
      detail: "+12% dibanding kemarin",
    },
    {
      label: "Produk Aktif",
      value: isLoadingProducts ? "…" : String(products.length),
      icon: "local_cafe",
      detail: `${categoryOptions.length} kategori`,
    },
    {
      label: "Meja Online",
      value: `${activeTablesCount}/${Math.max(tables.length, 1)}`,
      icon: "table_restaurant",
      detail: "QR siap digunakan",
    },
  ];
  const analyticsSeries = [
    { label: "Signature", value: 82 },
    { label: "Seasonal", value: 64 },
    { label: "Merch", value: 38 },
    { label: "New Menu", value: 54 },
  ];
  const masterProductSummary = useMemo(() => {
    const counts = new Map<CategorySlug, number>();
    DEFAULT_ADMIN_PRODUCTS.forEach((product) => {
  ...
  const masterCatalogSize = DEFAULT_ADMIN_PRODUCTS.length;
  const layoutTables = tables.length
    ? tables.map((table) => ({
        name: table.name,
        seats: 4,
        status: table.active ? "available" : "offline",
      }))
    : DEFAULT_TABLE_LAYOUT;
  const offlineTablesCount = layoutTables.filter((table) => table.status === "offline").length;
  const reservedTablesCount = layoutTables.filter((table) => table.status === "reserved").length;
  ...
  useEffect(() => {
    ...
    let resolvedProducts: AdminProductRecord[] = loadAdminProductsFromStorage();
  ...
  const toggleBulkChannel = (channel: BulkChannelKey) => {
  ...
  const handleSendBulkMessage = () => {
    if (!bulkMessage.trim()) {
      setBulkStatus("Isi pesan terlebih dahulu sebelum dijadwalkan.");
      return;
    }
    setBulkStatus(
      `Pesan untuk ${selectedSegments.length} segmen dijadwalkan melalui ${
        Object.entries(bulkChannels)
    ...
    setBulkMessage("");
    window.setTimeout(() => setBulkStatus(null), 2600);
  };

  const handleVoucherFieldChange = (field: keyof VoucherFormState, value: string) => {
    setVoucherForm((prev) => ({ ...prev, [field]: value }));
  };

  const handleVoucherSubmit = () => {
    if (!voucherForm.code.trim()) {
      setSaveMessage("Masukkan kode voucher terlebih dahulu.");
      window.setTimeout(() => setSaveMessage(null), 1800);
      return;
    }
    setVouchers((prev) => [
      {
        ...
        label:
          voucherForm.description.trim() ||
          (voucherForm.type === "percent"
            ? `Diskon ${voucherForm.value}%`
            : `Diskon Rp ${voucherForm.value}`),
        quota: 100,
        status: "Aktif",
      },
      ...prev,
    ...
    setVoucherForm({
      ...
      minSpend: voucherForm.minSpend,
      expiry: "",
      description: "",
    });
    setSaveMessage("Voucher baru ditambahkan.");
    window.setTimeout(() => setSaveMessage(null), 1800);
  };
                  ...
                  <div className="space-y-5 rounded-2xl border border-emerald-50 bg-white/90 p-5 shadow-sm">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          Pilih varian dari daftar berikut. Pengelolaan detail hanya dapat dilakukan dari menu Varian.
                        </p>
                      </div>
                      <p className="text-xs font-semibold text-gray-400">{variantTemplates.length} varian tersedia</p>
                    </div>
                    {variantTemplates.length ? (
                      <div className="flex flex-wrap gap-2">
                        {variantTemplates.map((variant) => {
                          const isApplied = productForm.customizations.some((group) => group.sourceVariantId === variant.id);
                          return (
                            <button
                              key={variant.id}
                              type="button"
                              onClick={() => applyVariantToProduct(variant.id)}
                              disabled={isApplied}
                      ...
                      <p className="rounded-2xl border border-dashed border-amber-200 bg-amber-50/60 px-4 py-3 text-xs font-semibold text-amber-600">
                        Belum ada varian. Kelola varian terlebih dahulu pada menu Varian.
                      </p>
                    )}
                    <div className="space-y-3">
                      {productForm.customizations.length ? (
                        productForm.customizations.map((group, index) => (
                          <div
                            key={group.id}
                            className="rounded-2xl border border-emerald-100 bg-white/95 px-4 py-3 shadow-sm"
                          >
                            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                              <div>
                                ...
                                <p className="text-xs text-gray-500">
                                  ...
                                  {group.required ? " • Wajib" : " • Opsional"} • Set #{index + 1}
                                </p>
                                {group.helperText ? (
                                  <p className="mt-1 text-xs text-gray-400">{group.helperText}</p>
                                ) : null}
                              ...
                              <button
                                ...
                                className="inline-flex items-center justify-center rounded-full border border-red-200 bg-red-50 px-3 py-1 text-xs font-semibold text-red-500 shadow-sm transition hover:bg-red-100"
                              >
                                Hapus Varian
                              </button>
                            </div>
                        ...
                        <p className="rounded-2xl border border-dashed border-emerald-200 bg-emerald-50/50 px-4 py-3 text-xs font-semibold text-emerald-600">
                          Belum ada varian diterapkan pada produk ini.
                        </p>
                      )}
                    </div>
                  </div>
  ...
  return (
    <>
      <div className="min-h-screen bg-gradient-to-br from-[#f8fafc] via-white to-[#eef6f3]">
        ...
        <div className="flex w-full items-center justify-between px-6 py-4 2xl:px-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h1 className="text-lg font-semibold text-gray-700">AIVRA Dashboard</h1>
              </div>
            </div>
        ...
        <div className="grid w-full gap-6 px-6 py-8 lg:grid-cols-[260px_1fr] 2xl:grid-cols-[300px_1fr] 2xl:px-10">
        <aside className="rounded-3xl bg-white/80 backdrop-blur shadow border border-emerald-50/70 p-5 space-y-6">
            ...
            <div key={section.title} className="space-y-2">
              ...
              <div className="space-y-1">
                {section.items.map((item) => {
                  ...
                  const href = getAdminSectionRoute(item.key);
                  return (
                    <Link
                      key={item.key}
                      href={href}
        ...
        <main className="rounded-3xl bg-white/85 backdrop-blur shadow border border-emerald-50/70 p-6 space-y-6">
            ...
            <section className="space-y-6">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Pantau pesanan, pendapatan, dan aktivitas tim secara real-time.
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-3">
                  <Link
                    href="/cashier"
                    className="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-white/80 px-4 py-2 text-xs font-semibold text-emerald-600 hover:bg-emerald-50 transition"
                  >
              ...
              <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
                <div className="space-y-6">
                  <div className="grid gap-4 md:grid-cols-2">
                    {summaryCards.map((stat) => (
                      <div
                        key={stat.label}
                        className="rounded-3xl border border-emerald-100 bg-gradient-to-br from-emerald-50 via-white to-emerald-100/70 p-5 shadow-sm"
                      >
                  ...
                  <div className="rounded-3xl border border-gray-100 bg-white/80 p-6 shadow-sm">
                    <div className="flex flex-col gap-2">
                      <div>
                        <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Order Pipeline</p>
                        <h3 className="text-lg font-semibold text-gray-800">Monitoring Pesanan</h3>
                        <p className="text-sm text-gray-500">
                          {pendingOrders + preparingOrders + readyOrders} pesanan aktif dalam antrean.
                        </p>
                      </div>
                    </div>
                    <div className="mt-5 grid gap-3 sm:grid-cols-3">
                      {[
                        { label: "Menunggu", value: pendingOrders, color: "bg-amber-400" },
                        { label: "Diproses", value: preparingOrders, color: "bg-sky-400" },
                        { label: "Siap", value: readyOrders, color: "bg-emerald-400" },
                      ].map((item) => (
                        <div
                          key={item.label}
                          className="rounded-2xl border border-gray-100 bg-gray-50/60 p-4 text-center"
                        >
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <Link
                      ...
                      className="group rounded-3xl border border-emerald-100 bg-gradient-to-br from-emerald-50 via-white to-emerald-100/70 p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
                    >
                      <div className="flex items-start justify-between">
                        <div>
                          <p className="text-xs uppercase tracking-[0.3em] text-emerald-500">Insights</p>
                          <h3 className="text-lg font-semibold text-gray-900">Laporan Penjualan</h3>
                          <p className="text-sm text-gray-600">
                            Buka detail revenue harian, per-pelanggan, dan performa channel.
                          </p>
                        </div>
                        <span className="material-symbols-outlined text-emerald-500 transition group-hover:translate-x-1">
                          trending_up
                        </span>
                      </div>
                  ...
                  <div className="rounded-3xl border border-emerald-100 bg-gradient-to-br from-white via-emerald-50 to-white p-6 shadow-sm">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Menu Analytics</p>
                        <h3 className="text-lg font-semibold text-gray-800">Performa Kategori</h3>
                      </div>
                    ...
                    <div className="mt-6 flex items-end gap-4">
                      {analyticsSeries.map((item) => (
                        <div key={item.label} className="flex flex-col items-center gap-2 text-xs text-gray-500">
                          <div className="relative h-28 w-10 rounded-full bg-emerald-50">
                            <div
                              className="absolute bottom-0 left-0 right-0 rounded-full bg-gradient-to-t from-emerald-500 to-emerald-300"
                ...
                <div className="space-y-4">
                  <div className="rounded-3xl border border-gray-100 bg-white/85 p-6 shadow-sm">
                    <div className="flex items-center justify-between">
                      <h3 className="text-lg font-semibold text-gray-800">Kolaborasi Tim</h3>
                    </div>
                    <div className="mt-5 space-y-3">
                      {collaborationMembers.map((member) => {
                        const presence = resolvePresenceStatus(presenceMap[member.email], nowTick);
                        return (
                          <div key={member.email} className="flex items-center justify-between rounded-2xl border border-gray-100 px-4 py-3">
                            <div className="flex items-center gap-3">
                              <div className="h-10 w-10 rounded-full bg-emerald-100 text-sm font-semibold text-emerald-700 grid place-items-center">
                            ...
                            <span
                              className={`rounded-full px-3 py-1 text-xs font-semibold ${presence.className}`}
                            >
                              {presence.label}
                            </span>
                          </div>
                  ...
                  <div className="rounded-3xl border border-emerald-100 bg-gradient-to-br from-emerald-500 via-emerald-400 to-emerald-600 p-6 text-white shadow-lg space-y-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-xs uppercase tracking-[0.5em] text-white/70">Progress</p>
                        <h3 className="text-lg font-semibold">Layanan & Fulfillment</h3>
                      </div>
                      <span className="material-symbols-outlined text-white/80">av_timer</span>
                    </div>
                    <div className="grid gap-3 sm:grid-cols-3 text-xs text-white/80">
                      <label className="flex flex-col gap-2 sm:col-span-2">
                        <span className="uppercase tracking-[0.3em] text-white/60">Target pesanan harian</span>
                        <div className="flex items-center gap-2 rounded-2xl bg-white/10 px-4 py-2">
                          ...
                          <input
                            ...
                            onChange={(event) => setDailyOrderTarget(Math.max(1, Number(event.target.value) || 1))}
                            className="w-full bg-transparent text-white placeholder-white/70 focus:outline-none"
                            placeholder="Contoh: 30"
                          />
                        </div>
                      </label>
                      <div className="rounded-2xl bg-white/10 px-4 py-3 text-center text-sm">
                        <p className="text-white/70">Capai</p>
                        <p className="text-white font-semibold">{readyOrders + servedOrders}</p>
                      </div>
                    </div>
                    <div className="mt-6 flex items-center gap-6">
                      <div className="relative h-32 w-32">
                        <div
                          className="absolute inset-0 rounded-full border-[10px] border-white/20"
                          style={{
                        ...
                        <div
                          ...
                          style={{
                            background: `conic-gradient(#ffffff ${fulfillmentRate * 3.6}deg, rgba(255,255,255,0.2) 0deg)`,
                          }}
                        />
                        <div className="absolute inset-4 rounded-full bg-emerald-500/40 grid place-items-center">
                          <div className="text-center">
                            <p className="text-3xl font-semibold">{fulfillmentRate}%</p>
                            <p className="text-xs uppercase tracking-[0.3em] text-white/70">Selesai</p>
                          </div>
                        </div>
                      </div>
                      <div className="space-y-2 text-sm">
                        <p className="text-white/80">
                          {servedOrders} dari {fulfillmentTarget} pesanan selesai hari ini. Pertahankan SLA &lt; 12 menit.
                        </p>
                        <div className="rounded-2xl bg-white/10 px-4 py-2 text-xs">
                          Timer layanan: <span className="font-semibold">01:24:08</span>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {activeKey === "menu-variants" ? (
            <section className="space-y-6">
              <div className="flex flex-col gap-2">
                <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Menu & Varian</p>
                <h2 className="text-xl font-semibold text-gray-800">Template Varian Favorit</h2>
                <p className="text-sm text-gray-500">
                  Kelola seluruh varian tambahan seperti ukuran, level es, atau topping dari halaman ini.
                </p>
              </div>
              {variantMessage ? (
                <p className="rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-2 text-xs font-semibold text-emerald-600">
                  {variantMessage}
                </p>
              ) : null}
              <div className="grid gap-4 lg:grid-cols-2">
                {variantTemplates.map((variant) => (
                  <div
                    key={variant.id}
                    className="rounded-3xl border border-emerald-100 bg-white/80 p-6 shadow-sm flex flex-col gap-4"
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Template</p>
                        <h3 className="text-lg font-semibold text-gray-800">{variant.name}</h3>
                        {variant.helperText ? (
                          <p className="text-sm text-gray-500">{variant.helperText}</p>
                        ) : (
                          <p className="text-sm text-gray-500">Gunakan varian ini untuk konsistensi permintaan pelanggan.</p>
                        )}
                      </div>
                    ...
                    <div className="space-y-3 text-sm text-gray-600">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {variant.options.map((option) => (
                          ...
                          >
                            {option.label}
                            {option.priceAdjustment !== 0
                              ? ` (${option.priceAdjustment > 0 ? "+" : ""}${currencyFormatter.format(
                      ...
                      <p className="text-xs text-emerald-500">
                        {variant.required ? "Wajib dipilih" : "Opsional"} • {variant.options.length} opsi tersedia
                      </p>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        className="rounded-full border border-emerald-200 bg-white px-3 py-1 text-xs font-semibold text-emerald-600 shadow-sm transition hover:bg-emerald-50"
                      >
                        Edit
                      </button>
                      <button
                ...
                {variantTemplates.length === 0 ? (
                  <div className="rounded-3xl border border-dashed border-emerald-200 bg-emerald-50/60 p-6 text-sm text-emerald-600">
                    Belum ada varian. Tambahkan varian baru melalui form di bawah ini.
                  </div>
                ) : null}
              </div>
              ...
              <div className="rounded-3xl border border-emerald-100 bg-white/90 p-6 shadow-sm space-y-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs uppercase tracking-[0.3em] text-gray-400">
                      {editingVariantId ? "Edit Varian" : "Tambah Varian"}
                    </p>
                    <h3 className="text-lg font-semibold text-gray-800">
                      {editingVariantId ? "Perbarui Varian" : "Buat Varian Baru"}
                    </h3>
                    <p className="text-sm text-gray-500">
                      Lengkapi detail berikut agar opsi tambahan pelanggan konsisten di seluruh produk.
                    </p>
                  </div>
                  {editingVariantId ? (
                    <button
                      ...
                      className="rounded-full border border-gray-200 bg-white px-3 py-1 text-xs font-semibold text-gray-500 hover:bg-gray-50"
                    >
                      Batal Edit
                    </button>
                  ) : null}
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <label className="flex flex-col gap-2 text-sm text-gray-600">
                    ...
                    <input
                      ...
                      onChange={(event) => handleVariantTextFieldChange("name", event.target.value)}
                      placeholder="Contoh: Level Gula"
                      className="rounded-xl border border-emerald-100 bg-white px-4 py-3 text-sm text-gray-700 shadow-sm focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                    />
                  </label>
                  <label className="flex flex-col gap-2 text-sm text-gray-600">
                    ...
                    <input
                      value={variantForm.helperText}
                      onChange={(event) => handleVariantTextFieldChange("helperText", event.target.value)}
                      placeholder="Contoh: Sesuaikan preferensi pelanggan"
                      className="rounded-xl border border-emerald-100 bg-white px-4 py-3 text-sm text-gray-700 shadow-sm focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                    />
                  </label>
                  <label className="flex flex-col gap-2 text-sm text-gray-600">
                    ...
                    <select
                      ...
                      onChange={(event) => handleVariantTypeChange(event.target.value as CustomizationGroup["type"])}
                      className="rounded-xl border border-emerald-100 bg-white px-4 py-3 text-sm text-gray-700 shadow-sm focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                    >
                  ...
                  <label className="inline-flex items-center gap-2 rounded-xl border border-emerald-100 bg-emerald-50/70 px-3 py-2 text-sm text-gray-600 shadow-sm">
                    <input
                      type="checkbox"
                      checked={variantForm.required}
                      onChange={(event) => handleVariantRequiredChange(event.target.checked)}
                ...
                <div className="space-y-3">
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-gray-500">Daftar Opsi</p>
                  {variantForm.options.map((option) => (
                    <div
                      key={option.id}
                      className="rounded-2xl border border-emerald-100 bg-white px-4 py-3 shadow-sm flex flex-col gap-3 md:flex-row md:items-start md:gap-4"
                    >
                      <label className="flex-1 text-xs font-semibold uppercase tracking-[0.2em] text-gray-500">
                        Nama Pilihan
                        <input
                          value={option.label}
                          onChange={(event) => handleVariantOptionFieldChange(option.id, "label", event.target.value)}
                      ...
                      <label className="w-full md:w-48 text-xs font-semibold uppercase tracking-[0.2em] text-gray-500">
                        Penyesuaian Harga
                        <div className="mt-1 flex items-center gap-2 rounded-lg border border-emerald-100 bg-white px-3 py-2 shadow-inner">
                          ...
                          <input
                            ...
                            onChange={(event) => handleVariantOptionFieldChange(option.id, "priceAdjustment", event.target.value)}
                      ...
                      <button
                        type="button"
                        onClick={() => removeVariantOptionField(option.id)}
                ...
                <div className="flex flex-wrap justify-end gap-3">
                  ...
                  <button
                    ...
                    className="rounded-full border border-emerald-200 bg-emerald-500 px-4 py-2 text-xs font-semibold text-white shadow hover:bg-emerald-600"
                  >
          ...
          {activeKey === "menu-categories" ? (
            <section className="space-y-6">
              <div className="flex flex-col gap-2">
                <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Menu & Kategori</p>
                <h2 className="text-xl font-semibold text-gray-800">Pengelompokan Produk</h2>
                <p className="text-sm text-gray-500">
                  Atur kategori agar menu digital dan kasir memiliki struktur yang sama.
                </p>
              </div>
              {categoryMessage ? (
                <p className="rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-2 text-xs font-semibold text-emerald-600">
              ...
              <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
                <div className="space-y-3">
                  {categoryOptions.map((slug) => {
                    const iconValue = resolveCategoryIcon(slug, categoryIcons);
                    const productCount = products.filter((product) => product.category === slug).length;
                    return (
                      <div
                        key={slug}
                        className="rounded-3xl border border-emerald-100 bg-white/80 px-5 py-4 shadow-sm flex flex-col gap-3"
                      >
                        <div className="flex flex-wrap items-center gap-3">
                          ...
                          <div className="flex-1 min-w-[140px]">
                            ...
                            <p className="text-xs text-gray-500">
                              {productCount} produk • slug: <code className="font-mono">{slug}</code>
                            </p>
                          </div>
                          <select
                            value={iconValue}
                            onChange={(event) => setCategoryIconValue(slug, event.target.value)}
                            className="rounded-xl border border-emerald-100 bg-white px-3 py-2 text-sm text-gray-700 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                          >
                ...
                <div className="rounded-3xl border border-emerald-100 bg-gradient-to-br from-emerald-50 via-white to-emerald-100/70 p-5 shadow-inner space-y-4">
                  <div>
                    <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Tambah Kategori</p>
                    <h3 className="text-lg font-semibold text-gray-800">Manual Mapping</h3>
                    <p className="text-sm text-gray-500">
                      Kategori baru otomatis tersedia untuk form produk.
                    </p>
                  </div>
                  <label className="text-xs font-semibold uppercase tracking-[0.2em] text-gray-500">
                    ...
                    <input
                      ...
                      placeholder="Contoh: Seasonal Blend"
                      className="mt-2 w-full rounded-2xl border border-emerald-100 bg-white px-4 py-3 text-sm text-gray-700 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                    />
                  </label>
                  <label className="text-xs font-semibold uppercase tracking-[0.2em] text-gray-500">
                    Ikon
                  ...
                  <button
                    ...
                    className="w-full rounded-full bg-emerald-500 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-emerald-600 transition"
                  >
                    Simpan Kategori
                  </button>
                  <p className="text-xs text-emerald-600">
                    Gunakan huruf & angka saja — slug dibuat otomatis dari nama kategori.
                  </p>
                </div>
              </div>
            </section>
          ...
          {activeKey === "menu-master-products" ? (
            <section className="space-y-6">
              <div className="flex flex-col gap-2">
                <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Master Data</p>
                <h2 className="text-xl font-semibold text-gray-800">Master Produk Template</h2>
                <p className="text-sm text-gray-500">
                  Data dasar digunakan saat bootstrap store baru atau reset katalog.
                </p>
              </div>
              <div className="rounded-3xl border border-emerald-100 bg-gradient-to-r from-emerald-500 via-emerald-400 to-emerald-500 p-6 text-white shadow-lg flex flex-col gap-2">
                <p className="text-xs uppercase tracking-[0.4em] text-white/80">Master Catalog</p>
                <div className="flex flex-wrap items-center gap-6">
                  <div>
                    <p className="text-4xl font-semibold">{masterCatalogSize}</p>
                    <p className="text-sm text-white/80">produk siap pakai</p>
                  </div>
                  <div className="flex flex-col gap-1 text-sm text-white/80">
                    <span>• {DEFAULT_CATEGORY_OPTIONS.length} kategori awal</span>
                    <span>• {variantTemplates.length} template varian</span>
                    <span>• Disimpan di local storage untuk onboarding cepat</span>
                  </div>
                </div>
              </div>
              <div className="rounded-3xl border border-emerald-50 overflow-hidden">
                <table className="min-w-full divide-y divide-emerald-100 text-sm">
                  <thead className="bg-emerald-50 text-left text-xs font-semibold uppercase tracking-[0.25em] text-emerald-600">
                    <tr>
                      <th className="px-4 py-3">Kategori</th>
                      <th className="px-4 py-3">Jumlah Produk</th>
                      <th className="px-4 py-3">Contoh</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-emerald-50 bg-white/90">
                    {masterProductSummary.map((entry) => {
                      const sampleProduct =
                        DEFAULT_ADMIN_PRODUCTS.find((product) => product.category === entry.slug)?.name ??
                        "—";
                      return (
                        <tr key={entry.slug}>
                          <td className="px-4 py-3 font-medium text-gray-700">{entry.label}</td>
                          <td className="px-4 py-3 text-gray-600">{entry.count} menu</td>
                          <td className="px-4 py-3 text-gray-500">{sampleProduct}</td>
                        </tr>
                      );
                    })}
                  </tbody>
          ...
          {activeKey === "menu-master-variants" ? (
            <section className="space-y-6">
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Master Varian</p>
                <h2 className="text-xl font-semibold text-gray-800">Guideline Varian Global</h2>
                <p className="text-sm text-gray-500">
                  Ringkasan seluruh varian yang telah disetujui untuk menjaga konsistensi pengalaman pelanggan.
                </p>
              </div>
              <div className="grid gap-4 lg:grid-cols-2">
                {variantTemplates.map((variant) => (
                  <div
                    key={variant.id}
                    className="rounded-3xl border border-gray-100 bg-white/85 p-5 shadow-sm space-y-3"
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Template</p>
                      ...
                      <span className="rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-600">
                        {variant.type === "single" ? "Single Choice" : "Multi Choice"}
                      </span>
                    </div>
                    <p className="text-sm text-gray-500">
                      {variant.helperText || "Gunakan varian ini sebagai standar ketika menambah produk."}
                    </p>
                    <ul className="space-y-2 rounded-2xl border border-gray-100 bg-gray-50/70 p-4 text-sm text-gray-600">
                      {variant.options.map((option) => (
                        <li key={option.id} className="flex items-center justify-between gap-2">
                          ...
                          <span className="text-xs text-gray-500">
                            {option.priceAdjustment === 0
                              ? "Tanpa penyesuaian"
                              : currencyFormatter.format(option.priceAdjustment)}
                          </span>
                        </li>
                      ))}
                    </ul>
                    <p className="text-xs text-gray-400">
                      {variant.required ? "Wajib dipilih" : "Opsional"} • {variant.options.length} pilihan
                    </p>
                  </div>
                ))}
                {variantTemplates.length === 0 ? (
                  <div className="rounded-3xl border border-dashed border-gray-200 bg-gray-50/70 p-6 text-sm text-gray-500">
                    Belum ada varian ditambahkan. Gunakan tab Varian untuk membuatnya terlebih dahulu.
                  </div>
                ) : null}
              </div>
            </section>
          ...
          {activeKey === "menu-master-categories" ? (
            <section className="space-y-6">
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Master Kategori</p>
                <h2 className="text-xl font-semibold text-gray-800">Blueprint Struktur Menu</h2>
                <p className="text-sm text-gray-500">
                  Gunakan daftar ini sebagai referensi ketika membuka cabang baru atau reset data.
                </p>
              </div>
              <div className="grid gap-4 md:grid-cols-2">
                {DEFAULT_CATEGORY_OPTIONS.map((slug) => (
                  <div
                    key={slug}
                    className="rounded-3xl border border-gray-100 bg-white/80 p-5 shadow-sm flex items-center gap-4"
                  >
                    <span className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-600">
                      <CategoryIcon value={resolveCategoryIcon(slug, categoryIcons)} className="text-xl" />
                    </span>
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500">
                        {DEFAULT_ADMIN_PRODUCTS.filter((product) => product.category === slug).length} produk master
                      </p>
                    </div>
          ...
          {activeKey === "customer-list" ? (
            <section className="space-y-6">
              <div className="flex flex-col gap-2">
                <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Pelanggan</p>
                <h2 className="text-xl font-semibold text-gray-800">Profil Pelanggan Aktif</h2>
                <p className="text-sm text-gray-500">
                  Insight singkat untuk personalisasi kampanye dan layanan.
                </p>
              </div>
              <div className="grid gap-4 md:grid-cols-2">
                {CUSTOMER_PROFILES.map((customer) => (
                  <div key={customer.name} className="rounded-3xl border border-gray-100 bg-white/85 p-5 shadow-sm space-y-3">
                    <div className="flex items-center justify-between">
                      <div>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm text-gray-600">
                      <div>
                        <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Transaksi</p>
                        <p className="text-base font-semibold text-gray-800">{customer.transactions}x</p>
                      </div>
                      <div>
                        <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Total Belanja</p>
                        <p className="text-base font-semibold text-gray-800">{customer.spend}</p>
                      </div>
                    </div>
                    <div className="rounded-2xl border border-gray-100 bg-gray-50/70 px-4 py-2 text-sm text-gray-600">
                      Menu favorit: <span className="font-semibold text-gray-800">{customer.favorite}</span>
                    </div>
                    <div className="flex items-center gap-2 text-xs">
                      <button className="rounded-full border border-emerald-200 px-3 py-1 font-semibold text-emerald-600 hover:bg-emerald-50">
                        Kirim pesan
                      </button>
                      <button className="rounded-full border border-gray-200 px-3 py-1 font-semibold text-gray-500 hover:bg-gray-50">
                        Detail lengkap
                      </button>
                    </div>
                  </div>
          ...
          {activeKey === "customer-bulk" ? (
            <section className="space-y-6">
              <div className="flex flex-col gap-2">
                <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Pesan Massal</p>
                <h2 className="text-xl font-semibold text-gray-800">Broadcast & Segmentasi</h2>
                <p className="text-sm text-gray-500">
                  Kirim pengumuman promo atau informasi penting ke segmen pelanggan yang relevan.
                </p>
              </div>
              <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
                <div className="rounded-3xl border border-emerald-100 bg-white/85 p-6 shadow-sm space-y-4">
                  <div className="flex flex-wrap items-center gap-2">
                    {BULK_CHANNELS.map((channel) => {
                      const isActive = bulkChannels[channel.key];
                      return (
                        <button
                          key={channel.key}
                          type="button"
                          onClick={() => toggleBulkChannel(channel.key)}
                  ...
                  <textarea
                    ...
                    rows={8}
                    placeholder="Tulis pesan broadcast, contoh: “Halo #nama#, nikmati diskon 15% untuk menu favoritmu akhir pekan ini…”"
                    className="w-full rounded-3xl border border-emerald-100 bg-white px-4 py-3 text-sm text-gray-700 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                  />
                  <div className="flex flex-wrap items-center gap-3">
                    <button
                      type="button"
                      onClick={handleSendBulkMessage}
                      className="inline-flex items-center gap-2 rounded-full bg-emerald-500 px-5 py-2 text-xs font-semibold text-white shadow hover:bg-emerald-600 transition"
                    >
                      Jadwalkan Pesan
                      <span className="material-symbols-outlined text-sm">send</span>
                    </button>
                    <p className="text-xs text-gray-500">
                      Variabel dinamis yang tersedia: <code className="font-mono">#nama#</code>,{" "}
                      <code className="font-mono">#menu_favorit#</code>.
                    </p>
                  </div>
                  {bulkStatus ? (
                    <p className="rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-2 text-xs font-semibold text-emerald-600">
                ...
                <div className="rounded-3xl border border-gray-100 bg-gray-50/70 p-5 shadow-inner space-y-4">
                  <div>
                    <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Segmen</p>
                    <h3 className="text-lg font-semibold text-gray-800">Pilih Target Audiens</h3>
                  </div>
                  <div className="space-y-3">
                      ...
                      return (
                        <button
                          ...
                          className={`w-full rounded-2xl border px-4 py-3 text-left text-sm transition ${
                            isSelected
                              ? "border-emerald-200 bg-white text-gray-700 shadow"
                              : "border-gray-200 bg-gray-50 text-gray-500 hover:border-emerald-100"
                          }`}
                        >
                  ...
                  <div className="rounded-2xl border border-white/60 bg-white px-4 py-3 text-sm text-gray-600">
                    <p className="font-semibold text-gray-800">
                      {selectedSegments.length} segmen dipilih
                    </p>
                    <p className="text-xs text-gray-500">
                      Pastikan jadwal broadcast tidak bertabrakan dengan kampanye lain.
                    </p>
                  </div>
                </div>
              </div>
          ...
          {activeKey === "customer-voucher" ? (
            <section className="space-y-6">
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Voucher & Promo</p>
                <h2 className="text-xl font-semibold text-gray-800">Kelola Kupon Digital</h2>
                <p className="text-sm text-gray-500">
                  Atur voucher loyalitas, promo musiman, dan kode unik untuk partner.
                </p>
              </div>
              <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
                <div className="rounded-3xl border border-emerald-100 bg-white/85 p-6 shadow-sm space-y-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <label className="text-xs font-semibold uppercase tracking-[0.3em] text-gray-400">
                      ...
                      <input
                        ...
                        onChange={(event) => handleVoucherFieldChange("code", event.target.value.toUpperCase())}
                        className="mt-2 w-full rounded-xl border border-emerald-100 bg-white px-4 py-3 text-sm text-gray-700 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                    ...
                    <label className="text-xs font-semibold uppercase tracking-[0.3em] text-gray-400">
                      ...
                      <select
                        ...
                        onChange={(event) => handleVoucherFieldChange("type", event.target.value)}
                        className="mt-2 w-full rounded-xl border border-emerald-100 bg-white px-4 py-3 text-sm text-gray-700 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                      >
                  ...
                  <div className="grid gap-4 sm:grid-cols-3">
                    <label className="text-xs font-semibold uppercase tracking-[0.3em] text-gray-400">
                      ...
                      <input
                        value={voucherForm.value}
                        onChange={(event) => handleVoucherFieldChange("value", event.target.value)}
                        className="mt-2 w-full rounded-xl border border-emerald-100 bg-white px-4 py-3 text-sm text-gray-700 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                      />
                    </label>
                    <label className="text-xs font-semibold uppercase tracking-[0.3em] text-gray-400">
                      ...
                      <input
                        ...
                        onChange={(event) => handleVoucherFieldChange("minSpend", event.target.value)}
                        className="mt-2 w-full rounded-xl border border-emerald-100 bg-white px-4 py-3 text-sm text-gray-700 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                      />
                    </label>
                    <label className="text-xs font-semibold uppercase tracking-[0.3em] text-gray-400">
                      ...
                      <input
                        ...
                        onChange={(event) => handleVoucherFieldChange("expiry", event.target.value)}
                        className="mt-2 w-full rounded-xl border border-emerald-100 bg-white px-4 py-3 text-sm text-gray-700 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                      />
                    </label>
                  </div>
                  <label className="text-xs font-semibold uppercase tracking-[0.3em] text-gray-400">
                    ...
                    <textarea
                      ...
                      onChange={(event) => handleVoucherFieldChange("description", event.target.value)}
                      rows={4}
                      className="mt-2 w-full rounded-2xl border border-emerald-100 bg-white px-4 py-3 text-sm text-gray-700 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                      placeholder="Contoh: Diskon spesial untuk pelanggan loyal akhir pekan."
                    />
                  </label>
                  <button
                    type="button"
                ...
                <div className="space-y-3">
                  {vouchers.map((voucher) => (
                    <div
                      key={voucher.code}
                      className="rounded-3xl border border-gray-100 bg-white/85 p-5 shadow-sm flex flex-col gap-2"
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-lg font-semibold text-gray-800">{voucher.code}</p>
                          <p className="text-sm text-gray-500">{voucher.label}</p>
                        </div>
                        <span
                          className={`rounded-full px-3 py-1 text-xs font-semibold ${
                            voucher.status === "Aktif"
                              ? "bg-emerald-50 text-emerald-600"
                              : voucher.status === "Terjadwal"
                      ...
                      <p className="text-xs text-gray-500">Kuota tersisa {voucher.quota} klaim</p>
                      <div className="flex items-center gap-2 text-xs">
                        <button
                          className="rounded-full border border-emerald-200 px-3 py-1 font-semibold text-emerald-600 hover:bg-emerald-50"
                          onClick={() => handleCopyText(voucher.code, "Kode voucher")}
                        >
                          Salin kode
                        </button>
                        <button className="rounded-full border border-gray-200 px-3 py-1 font-semibold text-gray-500 hover:bg-gray-50">
          ...
          {activeKey === "table-overview" ? (
            <section className="space-y-6">
              <div className="flex flex-col gap-2">
                <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Manajemen Meja</p>
                <h2 className="text-xl font-semibold text-gray-800">Ringkasan Area Dine-in</h2>
                <p className="text-sm text-gray-500">
                  Pantau kapasitas meja, status reservasi, dan tugas housekeeping.
                </p>
              </div>
              <div className="grid gap-4 md:grid-cols-4">
                {[
                  { label: "Total Meja", value: layoutTables.length, icon: "table_restaurant" },
                  { label: "Reservasi aktif", value: reservedTablesCount, icon: "event" },
                  { label: "Butuh perhatian", value: offlineTablesCount, icon: "warning" },
                  { label: "QR aktif", value: activeTablesCount, icon: "qr_code_2" },
                ].map((stat) => (
                  <div key={stat.label} className="rounded-3xl border border-emerald-100 bg-white/80 p-4 shadow-sm">
                    <div className="flex items-center gap-3">
                      <span className="material-symbols-outlined rounded-2xl bg-emerald-50 p-2 text-emerald-500">
                        {stat.icon}
                      </span>
                      <div>
              ...
              <div className="grid gap-4 lg:grid-cols-[1.3fr_0.7fr]">
                <div className="rounded-3xl border border-gray-100 bg-white/85 p-5 shadow-sm">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Occupancy</p>
                      <h3 className="text-lg font-semibold text-gray-800">Perubahan traffic</h3>
                    </div>
                    <span className="rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-600">
                      Live Update
                    </span>
                  </div>
                  <div className="mt-4 grid grid-cols-4 gap-3 text-center text-sm text-gray-500">
                    {["09.00", "12.00", "15.00", "21.00"].map((time, idx) => (
                      <div key={time} className="space-y-2">
                        <div className="mx-auto h-16 w-10 rounded-full bg-emerald-50 relative">
                          <div
                            className="absolute bottom-0 left-0 right-0 rounded-full bg-emerald-400"
                            style={{ height: `${40 + idx * 15}%` }}
                          />
                        </div>
                        <p>{time}</p>
                      </div>
                ...
                <div className="rounded-3xl border border-gray-100 bg-white/90 p-5 shadow-sm space-y-3">
                  <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Tugas Operasional</p>
                  {TABLE_SERVICE_TASKS.map((task) => (
                    <div key={task.title} className="rounded-2xl border border-gray-100 bg-gray-50/70 px-4 py-3">
                      <p className="text-sm font-semibold text-gray-800">{task.title}</p>
                      <p className="text-xs text-gray-500">{task.owner}</p>
                      <span className="mt-2 inline-flex rounded-full bg-emerald-50 px-3 py-1 text-[11px] font-semibold text-emerald-600">
                        {task.status}
                      </span>
                    </div>
                  ))}
          ...
          {activeKey === "table-layout" ? (
            <section className="space-y-6">
              <div className="flex flex-col gap-2">
                <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Layout & Varian Meja</p>
                <h2 className="text-xl font-semibold text-gray-800">Monitoring Denah</h2>
                <p className="text-sm text-gray-500">
                  Gunakan daftar berikut untuk memastikan rotasi meja berjalan mulus.
                </p>
              </div>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {layoutTables.map((table) => (
                  <div
                    key={table.name}
                    className="rounded-3xl border border-gray-100 bg-white/85 p-5 shadow-sm space-y-2"
                  >
                    <div className="flex items-center justify-between">
                    ...
                    <p className="text-xs text-gray-500">Kapasitas {table.seats} orang</p>
                    <div className="flex items-center gap-2 text-xs">
                      <button className="rounded-full border border-emerald-200 px-3 py-1 font-semibold text-emerald-600 hover:bg-emerald-50">
                        Tanda selesai
                      </button>
            ...
            <section className="space-y-5">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                  ...
                  <h2 className="text-xl font-semibold text-gray-700">Portal QR Setiap Meja</h2>
                  <p className="text-sm text-gray-500 mt-1">
          ...
          {activeKey === "table-reservations" ? (
            <section className="space-y-6">
              <div className="flex flex-col gap-2">
                <p className="text-xs uppercase tracking-[0.3em] text-gray-400">Reservasi</p>
                <h2 className="text-xl font-semibold text-gray-800">Daftar Booking</h2>
                <p className="text-sm text-gray-500">
                  Koordinasikan antara host, barista, dan kitchen agar pengalaman tamu mulus.
                </p>
              </div>
              <div className="space-y-3">
                {TABLE_RESERVATIONS.map((booking) => (
                  <div
                    key={booking.guest}
                    className="rounded-3xl border border-gray-100 bg-white/80 px-5 py-4 shadow-sm flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"
                  >
                    <div>
                      <p className="text-lg font-semibold text-gray-800">{booking.guest}</p>
                      <p className="text-sm text-gray-500">
                        {booking.pax} orang • {booking.slot}
                      </p>
                      <p className="text-xs text-gray-400">{booking.note}</p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2 text-xs">
